      ...
      <div class="result-content">
        <table class="result-tab" width="100%">
          ...
          <?php if(is_array($prize_info)): $i = 0; $__LIST__ = $prize_info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$i): $mod = ($i % 2 );++$i;?><tr>
              <td><?php echo ($i["act_name"]); ?></td>
               <td><?php echo ($branch[$i['type_id']]); ?></td>
 				<td><?php echo (date("Y-m-d",$i['start_time'])); ?> </span></td>
                <td><?php echo (date("Y-m-d",$i['end_time'])); ?></td>
                <td><a href="#" onclick="update_status(<?php echo ($i["status"]); ?>,<?php echo ($i["act_id"]); ?>)"><?php if($i["status"] == 1): ?>未开启<?php else: ?>已开启<?php endif; ?></a></td>
                <td><a href="#" onclick="update(<?php echo ($i["act_id"]); ?>)">修改</a> 	| 	<a href="#" onclick="toufang(<?php echo ($i["act_id"]); ?>)">今日投放</a></td>
            </tr><?php endforeach; endif; else: echo "" ;endif; ?>
        </table>
        <div class="list-page">
          <ul class="pagination" >
            <li style=" float:left"> <a href="<?php echo U('index');?>?page=1">首页</a> </li>
            <?php $__FOR_START_13682__=$page;$__FOR_END_13682__=$page+4;for($i=$__FOR_START_13682__;$i < $__FOR_END_13682__;$i+=1){ if($i <= $pages) { ?>
...
function update_status(status,act_id){
			$.post("<?php echo U('upStatus');?>",{act_id:act_id,status:status},function(data,datatype){
...
layer.open({
  ...
  shade: 0.8,
  area: ['580px', '650px'],
  content: "<?php echo U('addAct');?>" //iframe的url
}); 
	...
	function update(id){
		...
		layer.open({
  			...
  			shade: 0.8,
  			area: ['580px', '580px'],
  			content: "<?php echo U('addAct');?>?id="+id //iframe的url
});	
		}
</script>